...
const ProfileCreate = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    rating: 4.2,
    bio: "Passionate athlete looking for weekly games, matches, and tournament squads.",
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      setError(res.message || "Failed to create sports profile.");
    }
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] py-8 px-4 sm:px-6 max-w-2xl mx-auto text-[#F5F0E6]">
      <div className="bg-court-900 border border-court-700 rounded-3xl p-6 sm:p-10 shadow-2xl relative overflow-hidden shadow-gold/10">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h2 className="text-3xl font-extrabold text-[#F5F0E6]">Create Your Athlete Profile</h2>
          <p className="text-xs text-[#9B9691] mt-2 max-w-md mx-auto">
            Set up your sports preferences to get your verified Digital Sports ID, match with nearby players, and join local tournaments.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            ...
            <div className="flex items-center gap-3">
              {SAMPLE_AVATARS.map((avatar, idx) => (
                <button
                  key={idx}
                  type="button"
                  onClick={() => setFormData({ ...formData, profilePhoto: avatar })}
                  className={`w-14 h-14 rounded-2xl overflow-hidden border-2 transition-all ${
                    formData.profilePhoto === avatar
                      ? "border-gold ring-2 ring-gold/40 scale-105 shadow-md shadow-gold/20"
                      : "border-court-700 hover:border-court-600 opacity-70"
                  }`}
                >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g. Evenings (5 PM - 8 PM)"
          ...
          <div>
            ...
            <textarea
              ...
              value={formData.bio}
              onChange={handleChange}
              placeholder="Tell other players about your playing style, achievements, or favorite positions..."
          ...
          >
            ...
            <span>{loading ? "Generating ID Pass..." : "Complete Profile & Enter Arena"}</span>
          </button>
        </form>
      </div>
    </div>
